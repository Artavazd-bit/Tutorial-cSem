...
To use the cSEM package users need to install R and an IDE, i.e. R-Studio.
We refer to the installation guide on the official website of R.

When opening RStudio for the first time the user is shown four panes. 
The Console pane lower left, where the code runs, the script editor upper left, where you input your code, the environment panes on the right hand side. 

First we need to create a new Rscript File, for this click on File -> New File -> R Script. 
The workflow is as follows: You write code and then run the lines with ctrl + enter.
When you want to write a comment use # in the beginning of the so the code does not run this line of code. 

To work with the package cSEM you need to install it first. 
The cSEM package is available on CRAN, thus the most recent stable version can be installed as followd:
\begin{lstlisting}
install.packages("cSEM")
\end{lstlisting}
This only needs to be done once, to update the package rerun this line. 
