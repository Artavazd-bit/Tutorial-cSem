...
12. **[Sec. assessment] Deduplicate the reliability chunks** — `assess-rho-A` and `assess-cronbach-alpha` both print `quality$Reliability`, which contains all three estimates (alpha, rho_C, rho_A) → identical table appears twice. Switch to element-level extraction (sub-element names to verify at knit). Also decide: keep all four rho_C variants (`quality$RhoC`, `_mm`, `_weighted`, `_weighted_mm` — names unverified, same status as `VIF_modeB`) or trim, since Benitez keys on rho_A. _Note 2026-08-04: the `assess()` API spells the criterion `vifmodeB`, so `quality$VIF_modeB` is likely the wrong element name — check the actual output element when settling this step._
