...
The output shows that the variables AC4 and EP4 each contain two missing values.
...
In particular, the \code{=\~} and the \code{<\~} operators are used to define the relations between the constructs and their indicators.
Specifically, the \code{=\~} operator is used to specify reflective measurement models, while the \code{<\~} is used to specify composite models \citep[see][for an explanation of reflective and composite models]{}.
In addition, the \code{\~} operator is used to specify the relations among constructs.
