...
\begin{description}
  ...
  \item[\prm{.model}] A model string written in \pkg{lavaan} syntax, with \code{$=/sim$} for measurement relations and \texttt{\texttildelow} for structural (regression) relations.
