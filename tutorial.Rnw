...
\begin{description}
  ...
  \item[\prm{.model}] A model string written in \pkg{lavaan} syntax, with \code{=\~} for measurement relations and \texttt{\texttildelow} for structural (regression) relations.
...
Following the guidelines described in \citet[Chapter~4]{hair_primer_2017} and \citet{sarstedt_partial_2022}, we assess the reflective measurement model in terms of internal consistency reliability, convergent validity, and discriminant validity. 
