...
The Employee Retention Model is stored as an CSV File using ';' as the separator, thus it can be loaded into R using following command:
...
In \pkg{cSEM}, models are specified using the \pkg{lavaan} package \citep{lavaan} model syntax.
...
Specifically, the \texttt{=\~} operator is used to specify reflective measurement models, while the \texttt{<\~} is used to specify composite models \citep[see][for an explanation of reflective and composite models]{lord_statistical_2008}.
