...
\begin{description}
  \item[\prm{.conv_criterion}] Character string specifying the criterion used to check for convergence of the weighting algorithm; one of \code{"diff_absolute"}, \code{"diff_squared"}, or \code{"diff_relative"}, with \code{"diff_absolute"} as the default.
  ...
  \item[\prm{.dominant_indicators}] A character vector of \code{"construct_name" = "indicator_name"} pairs naming the dominant indicator for a (subset of) construct(s), which fixes the sign/orientation of the corresponding composite; defaults to \code{NULL}.
  ...
  \item[\prm{.iter_max}] Integer giving the maximum number of iterations allowed for the weighting algorithm; with \code{.iter_max = 1} and \code{.approach_weights = "PLS-PM"} one-step weights are returned, and exceeding the maximum returns the weights of the last step with a warning (default \code{100}).
  ...
  \item[\prm{.PLS_approach_cf}] Character string selecting the approach used to obtain the correction factors for PLSc (e.g.\ \code{"dist_squared_euclid"}, \code{"dist_euclid_weighted"}, \code{"fisher_transformed"}, the various means, or \code{"geo_of_harmonic"}); defaults to \code{"dist_squared_euclid"} and is ignored unless disattenuation is applied with PLS-PM.
  \item[\prm{.PLS_ignore_structural_model}] Logical indicating whether the structural model should be ignored when computing the inner weights of the PLS-PM algorithm; defaults to \code{FALSE} and is ignored for non-PLS-PM weighting approaches.
  \item[\prm{.PLS_modes}] Specifies the mode used to estimate the weights of each construct in PLS-PM (e.g.\ \code{"modeA"}, \code{"modeB"}, \code{"modeBNNLS"}, \code{"unit"}, \code{"PCA"}, or user-supplied fixed weights), either as a named list per construct or a single value for all; defaults to \code{NULL}, in which case the appropriate mode is chosen automatically from the construct type.
  \item[\prm{.PLS_weight_scheme_inner}] Character string giving the inner weighting scheme used by PLS-PM; one of \code{"centroid"}, \code{"factorial"}, or \code{"path"}, with \code{"path"} as the default (ignored for non-PLS-PM approaches).
  \item[\prm{.reliabilities}] A character vector of \code{"name" = value} pairs supplying known reliabilities (between 0 and 1) for a (subset of) construct(s); defaults to \code{NULL}, in which case reliabilities are estimated by \code{csem()} (currently only supported for \code{.approach_weights = "PLS-PM"}).
  \item[\prm{.starting_values}] A named list of vectors of \code{"indicator_name" = value} pairs giving the (scaled or unscaled) starting indicator weights for the weighting algorithm of selected constructs; defaults to \code{NULL}.
