...
In particular, the \code{$=\sim$} and the \code{$<\sim$} operators are used to define the relations between the constructs and their indicators.
Specifically, the \code{$=\sim$} operator is used to specify reflective measurement models, while the \code{$<\sim$} is used to specify composite models \citep[see][for an explanation of reflective and composite models]{}.
In addition, the \code{$\sim$} operator is used to specify the relations among constructs.
...
\subsection{Adjusting the PLS Algorithm}
