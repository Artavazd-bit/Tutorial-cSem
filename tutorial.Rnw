...
To conduct statistical inference, the standard errors of the estimates must be obtained by a resampling procedure such as the bootstrap, which is introduced in Section~\ref{sec:assmess}.

\subsection{Assessing the measuement model}
\label{sec:assmess}

The next step involves assessing the measurment model results, according to the guidelines described in \citet{Benitez2020}
...
% TODO (Jason): lead-in text — assessment proceeds indicator -> construct -> between-construct;
% each criterion is extracted from `quality` in turn instead of printing the full output.

% ── Indicator level: loadings (indicator reliability) ─────────────────
% TODO (Jason): text — what a loading tells about indicator reliability, threshold 0.708, OC1 case.
...
% ── Construct level: internal consistency reliability ─────────────────
% TODO (Jason): text — Cronbach's alpha, rho_A (used by the PLSc correction), rho_C.
...
% ── Between constructs: discriminant validity ─────────────────────────
% TODO (Jason): text — HTMT, thresholds 0.85/0.90, decision via bootstrap CI upper bound
% -> natural transition to introducing the bootstrap (res_csem_boot).
